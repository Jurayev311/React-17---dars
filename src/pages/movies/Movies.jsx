...
const Movies = () => {
  ...
  return (
    <>
      <div id="genreContainer" className="container mx-auto max-w-[1250px] px-1.5 flex items-center justify-start gap-2 mb-4 mt-2 overflow-auto">
        {genreData?.genres?.map((genre) => (
          <div 
            ...
            className={`whitespace-nowrap px-3.5 py-1 cursor-pointer rounded-xl active:scale-95 duration-200 
              ${with_genres.split("-").includes(genre.id.toString()) ? "bg-red-600 text-white" : "dark:bg-slate-900 dark:text-white bg-slate-300"}`}
          >
            {genre.name}
          </div>
        ))}
